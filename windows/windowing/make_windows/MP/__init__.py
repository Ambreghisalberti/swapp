...
def intersect_MP(df, pos, omni, win_length, method, **kwargs):
    if method == 'encountersMSPandMSH':
        prepare_df_MSP_MSH_overlap(df, win_length)
    elif method == 'Shue':
...
def prepare_df_MSP_MSH_overlap(df, win_length):
    """
    ...
    pred_boosting(data)
    regions(data)
    flag_msp_and_msh(data, win_length)
...
def prepare_df_close_to_MP(pos, omni, win_length, **kwargs):
    ...
    flag_around_mp(pos_omni, win_length)
